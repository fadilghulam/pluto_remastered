...
func TestGPT(c *fiber.Ctx) error {
	...
	return c.JSON(resp.Choices[0].Content)
}
...
func executeToolCalls(ctx context.Context, llm llms.Model, messageHistory []llms.MessageContent, resp *llms.ContentResponse) []llms.MessageContent {
	...
	for _, toolCall := range resp.Choices[0].ToolCalls {
		...
		case "getCurrentWeather2":
			...
			response, err := getCurrentWeather2(args.Location, args.Unit)
			if err != nil {
				log.Fatal(err)
			}
...
func getCurrentWeather2(location string, unit string) (string, error) {
