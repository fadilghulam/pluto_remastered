...
func TestOllama3(c *fiber.Ctx) error {
	flag.Parse()
	...
	msgs = append(msgs, llms.TextParts(llms.ChatMessageTypeHuman, body.Question))
	...
	return c.JSON(fiber.Map{"response": msgs})
}
...
func dispatchCall(c *Call) (llms.MessageContent, bool) {
	...
	case "getCurrentCapital":
		loc, ok := c.Input["location"].(string)
		if !ok {
			log.Fatal("invalid input")
		}

		capital, err := getCurrentCapital(loc)
		if err != nil {
			log.Fatal(err)
		}

		return llms.TextParts(llms.ChatMessageTypeHuman, capital), true
	case "finalResponse":
...
func getCurrentCapital(location string) (string, error) {
	capitalInfo := map[string]any{
		"location": location,
		"capital":  "Jakarta",
	}
	b, err := json.Marshal(capitalInfo)
	if err != nil {
		return "", err
	}
	return string(b), nil
...
var functions = []llms.FunctionDefinition{
	...
	{
		Name:        "getCurrentCapital",
		Description: "Get the current capital of a given location",
		Parameters: json.RawMessage(`{
			"type": "object", 
			"properties": {
				"location": {"type": "string", "description": "The city and state, e.g. San Francisco, CA"}
